...
class ReadFiles {
  ...
  fun read(name: List<File>, inputStream: InputStream) {
    ...
    // There are three nice use cases to implement:
    // (1) Input: A .jar file (which is, under the hood, just a .zip file)
    //     Output: All the .class files inside the .jar are decompiled
    // (2) Input: A .tar file which contains many .jar files
    //     Output: All the .class files inside the .jar files decompiled
    // (3) Input: A URL to a .jar file and the name of a specific .class file
    //     Output: The specific .class file decompiled
    //
    // The problem is that for .zip files, you can append new files to them
    // and if you append a new file with the same name as an existing file,
    // the existing file is overwritten!! Hence, we need to extract all the
    // files from a .jar or .tar before we start processing the files.
    //
    // Note: Vulnerable to zip bombs, but that's a user problem. Don't run
    //       Jade on zip bombs!
