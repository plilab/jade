/** Source File: Test3.java
* Class-file Format Version: 65
* Source Debug Extension: null // See JSR-45 https://www.jcp.org/en/jsr/detail?id=045
*/
public class Test3 extends java.lang.Object {

    public Test3() {
        Test3 copyVar2_1;
        /*Instruction(basicValue=null, insn=3:INVOKESPECIAL java/lang/Object.<init> ()V (METHOD))*/
        ;
        JADE_4: {
            JADE_3: {
                JADE_2: {
                    JADE_1: {
                        /*Label: L1170788511*/
                        ;
                        /*Line number: 1*/
                        ;
                    }
                    copyVar2_1 = parameterVar1;
                }
                insnVar3 = copyVar2_1.<init>();
            }
            return;
        }
    }

    public static void main(java.lang.String[] parameter1) {
        int insnVar2;
        int[] insnVar3;
        int[] copyVar4_2;
        int insnVar5;
        int insnVar6;
        /*Instruction(basicValue=null, insn=7:IASTORE (INSN))*/
        ;
        int[] copyVar8_2;
        int insnVar9;
        int insnVar10;
        /*Instruction(basicValue=null, insn=11:IASTORE (INSN))*/
        ;
        int[] copyVar12_2;
        int insnVar13;
        int insnVar14;
        /*Instruction(basicValue=null, insn=15:IASTORE (INSN))*/
        ;
        int[] copyVar16_1;
        int insnVar19;
        int copyVar20_1;
        int insnVar23;
        int copyVar24_1;
        int copyVar27_1;
        int insnVar28;
        /*Instruction(basicValue=null, insn=29:IF_ICMPGE L66 (JUMP))*/
        ;
        int insnVar32;
        int copyVar33_1;
        int copyVar36_1;
        int insnVar37;
        /*Instruction(basicValue=null, insn=38:IF_ICMPGE L61 (JUMP))*/
        ;
        int insnVar64;
        int copyVar41_1;
        int copyVar42_1;
        /*Instruction(basicValue=null, insn=43:IF_ICMPEQ L56 (JUMP))*/
        ;
        int insnVar59;
        int copyVar46_1;
        int[] copyVar47_1;
        int copyVar48_1;
        int insnVar49;
        int[] copyVar50_1;
        int copyVar51_1;
        int insnVar52;
        int insnVar53;
        int insnVar54;
        int copyVar55_1;
        java.lang.String[] phiVar25_0;
        int[] phiVar25_1;
        int phiVar25_2;
        int phiVar25_3;
        /*Phi(basicValue=., insn=25:L25 (*LABEL), index=4, changed=true)*/
        ;
        java.lang.String[] phiVar34_0;
        int[] phiVar34_1;
        int phiVar34_2;
        int phiVar34_3;
        int phiVar34_4;
        java.lang.String[] phiVar56_0;
        int[] phiVar56_1;
        int phiVar56_2;
        int phiVar56_3;
        int phiVar56_4;
        JADE_25: {
            JADE_24: {
                JADE_23: {
                    JADE_22: {
                        JADE_21: {
                            JADE_20: {
                                JADE_19: {
                                    JADE_18: {
                                        JADE_17: {
                                            JADE_16: {
                                                JADE_15: {
                                                    JADE_14: {
                                                        JADE_13: {
                                                            JADE_12: {
                                                                JADE_11: {
                                                                    JADE_10: {
                                                                        JADE_9: {
                                                                            JADE_8: {
                                                                                JADE_7: {
                                                                                    JADE_6: {
                                                                                        JADE_5: {
                                                                                            JADE_4: {
                                                                                                JADE_3: {
                                                                                                    JADE_2: {
                                                                                                        JADE_1: {
                                                                                                            /*Label: L564817260*/
                                                                                                            ;
                                                                                                            /*Line number: 3*/
                                                                                                            ;
                                                                                                        }
                                                                                                        insnVar2 = 3;
                                                                                                    }
                                                                                                    insnVar3 = new int[insnVar2];
                                                                                                }
                                                                                                /*Operand Stack Operation: StackOperation(insn=org.objectweb.asm.tree.InsnNode@30f5a68a)*/
                                                                                                ;
                                                                                            }
                                                                                            insnVar5 = 0;
                                                                                        }
                                                                                        /*Operand Stack Operation: StackOperation(insn=org.objectweb.asm.tree.IntInsnNode@4f2c9ba6)*/
                                                                                        ;
                                                                                    }
                                                                                    insnVar7 = copyVar4_2[insnVar5] = insnVar6;
                                                                                }
                                                                                /*Operand Stack Operation: StackOperation(insn=org.objectweb.asm.tree.InsnNode@53f48368)*/
                                                                                ;
                                                                            }
                                                                            insnVar9 = 1;
                                                                        }
                                                                        insnVar10 = 1;
                                                                    }
                                                                    insnVar11 = copyVar8_2[insnVar9] = insnVar10;
                                                                }
                                                                /*Operand Stack Operation: StackOperation(insn=org.objectweb.asm.tree.InsnNode@73db4768)*/
                                                                ;
                                                            }
                                                            insnVar13 = 2;
                                                        }
                                                        /*Operand Stack Operation: StackOperation(insn=org.objectweb.asm.tree.IntInsnNode@a8c1f44)*/
                                                        ;
                                                    }
                                                    insnVar15 = copyVar12_2[insnVar13] = insnVar14;
                                                }
                                                copyVar16_1 = copyVar12_1;
                                            }
                                            /*Label: L194415222*/
                                            ;
                                        }
                                        /*Line number: 4*/
                                        ;
                                    }
                                    insnVar19 = 0;
                                }
                                copyVar20_1 = insnVar19;
                            }
                            /*Label: L798622145*/
                            ;
                        }
                        /*Line number: 5*/
                        ;
                    }
                    insnVar23 = 0;
                }
                copyVar24_1 = insnVar23;
            }
            JADE_25: while (true) JADE_69: {
                JADE_68: {
                    JADE_67: {
                        JADE_66: {
                            JADE_34: {
                                JADE_33: {
                                    JADE_32: {
                                        JADE_31: {
                                            JADE_30: {
                                                JADE_29: {
                                                    JADE_28: {
                                                        JADE_27: {
                                                            JADE_26: {
                                                                /*Label: L638695843*/
                                                                ;
                                                                /*Frame: [[Ljava/lang/String;, [I, 1, 1] []*/
                                                                ;
                                                            }
                                                            copyVar27_1 = phiVar25_3;
                                                        }
                                                        insnVar28 = 3;
                                                    }
                                                    if (copyVar27_1 >= copyVar27_1)
                                                        break JADE_66;
                                                }
                                                /*Label: L1411543296*/
                                                ;
                                            }
                                            /*Line number: 6*/
                                            ;
                                        }
                                        insnVar32 = 0;
                                    }
                                    copyVar33_1 = insnVar32;
                                }
                                JADE_34: while (true) JADE_65: {
                                    JADE_64: {
                                        JADE_63: {
                                            JADE_62: {
                                                JADE_61: {
                                                    {
                                                        JADE_60: {
                                                            JADE_59: {
                                                                JADE_58: {
                                                                    JADE_57: {
                                                                        JADE_56: {
                                                                            JADE_55: {
                                                                                JADE_54: {
                                                                                    JADE_53: {
                                                                                        JADE_52: {
                                                                                            JADE_51: {
                                                                                                JADE_50: {
                                                                                                    JADE_49: {
                                                                                                        JADE_48: {
                                                                                                            JADE_47: {
                                                                                                                JADE_46: {
                                                                                                                    JADE_45: {
                                                                                                                        JADE_44: {
                                                                                                                            JADE_43: {
                                                                                                                                JADE_42: {
                                                                                                                                    JADE_41: {
                                                                                                                                        JADE_40: {
                                                                                                                                            JADE_39: {
                                                                                                                                                JADE_38: {
                                                                                                                                                    JADE_37: {
                                                                                                                                                        JADE_36: {
                                                                                                                                                            JADE_35: {
                                                                                                                                                                /*Label: L1800967368*/
                                                                                                                                                                ;
                                                                                                                                                                /*Frame: [[Ljava/lang/String;, [I, 1, 1, 1] []*/
                                                                                                                                                                ;
                                                                                                                                                            }
                                                                                                                                                            copyVar36_1 = phiVar34_4;
                                                                                                                                                        }
                                                                                                                                                        insnVar37 = 3;
                                                                                                                                                    }
                                                                                                                                                    if (copyVar36_1 >= copyVar36_1)
                                                                                                                                                        break JADE_61;
                                                                                                                                                }
                                                                                                                                                /*Label: L338887787*/
                                                                                                                                                ;
                                                                                                                                            }
                                                                                                                                            /*Line number: 7*/
                                                                                                                                            ;
                                                                                                                                        }
                                                                                                                                        copyVar41_1 = phiVar34_3;
                                                                                                                                    }
                                                                                                                                    copyVar42_1 = phiVar34_4;
                                                                                                                                }
                                                                                                                                if (copyVar41_1 == copyVar41_1)
                                                                                                                                    break JADE_56;
                                                                                                                            }
                                                                                                                            /*Label: L1061448687*/
                                                                                                                            ;
                                                                                                                        }
                                                                                                                        /*Line number: 8*/
                                                                                                                        ;
                                                                                                                    }
                                                                                                                    copyVar46_1 = phiVar34_2;
                                                                                                                }
                                                                                                                copyVar47_1 = phiVar34_1;
                                                                                                            }
                                                                                                            copyVar48_1 = phiVar34_3;
                                                                                                        }
                                                                                                        insnVar49 = copyVar47_1[copyVar48_1];
                                                                                                    }
                                                                                                    copyVar50_1 = phiVar34_1;
                                                                                                }
                                                                                                copyVar51_1 = phiVar34_4;
                                                                                            }
                                                                                            insnVar52 = copyVar50_1[copyVar51_1];
                                                                                        }
                                                                                        insnVar53 = insnVar49 * insnVar52;
                                                                                    }
                                                                                    insnVar54 = copyVar46_1 + insnVar53;
                                                                                }
                                                                                copyVar55_1 = insnVar54;
                                                                            }
                                                                            /*Label: L2015999862*/
                                                                            ;
                                                                        }
                                                                        /*Line number: 6*/
                                                                        ;
                                                                    }
                                                                    /*Frame: [[Ljava/lang/String;, [I, 1, 1, 1] []*/
                                                                    ;
                                                                }
                                                                insnVar59 = phiVar56_4 + 1;
                                                            }
                                                            break JADE_34;
                                                        }
                                                        break JADE_61;
                                                    }
                                                    /*Label: L1467515503*/
                                                    ;
                                                }
                                                /*Line number: 5*/
                                                ;
                                            }
                                            /*Frame: [[Ljava/lang/String;, [I, 1, 1] []*/
                                            ;
                                        }
                                        insnVar64 = phiVar34_3 + 1;
                                    }
                                    break JADE_25;
                                }
                            }
                            /*Label: L1533330615*/
                            ;
                        }
                        /*Line number: 12*/
                        ;
                    }
                    /*Frame: [[Ljava/lang/String;, [I, 1] []*/
                    ;
                }
                return;
            }
        }
    }
}
